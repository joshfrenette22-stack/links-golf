    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 px-4 md:px-12 py-8 md:py-12">
        ...
        <div>
          ...
          <p className="text-[14px] text-[#7a7870] mb-4">{product.type}{product.spec ? ` · ${product.spec}` : ''}</p>
          <div className="mb-4"><ConditionBadge condition={product.condition} /></div>
          ...
          <button className="block w-full text-center bg-[#222] text-white text-[12px] font-bold uppercase tracking-[0.15em] py-5 hover:opacity-80 transition-opacity mt-2">
            Buy It Now
          </button>
        </div>
      </div>
      ...
      <div className="border-t border-[#e8e6e0] mt-8 pt-8 px-4 md:px-12 pb-12">
        ...
        <div className="max-w-[680px] text-[14px] leading-relaxed text-[#7a7870]">
          {activeTab === 0 && (
            <>
              <p className="mb-4">The {product.name} is a tour-proven {product.type.toLowerCase()} delivering exceptional performance for golfers of all skill levels. Built with premium materials and precision engineering, it offers the perfect blend of distance, control, and feel.</p>
              <p>Whether you&apos;re looking to add yards off the tee or improve your short game precision, this club will help you play your best golf. Pre-owned and graded {product.condition} — it performs exactly as described.</p>
            </>
          )}
          {activeTab === 1 && (
          ...
          {activeTab === 2 && (
            <dl className="grid grid-cols-2 gap-x-8 gap-y-3">
              {[['Loft', product.spec || 'Standard'], ['Lie', '60°'], ['Shaft', 'Stock'], ['Grip', 'Standard'], ['Length', 'Standard'], ['Dexterity', 'Right-Handed']].map(([k, v]) => (
                <div key={k}>
